...
Построенный экземпляр может быть немного другим, в отличие от ваших ожиданий.
...
Это объектные обертки играют очень важную цель. Дело в том что примитивы не имеют свойств и методов, А обертка позволяет нам обратиться к методом `.length` или `.toString()`. К счастью в JS эта обёртка создается автоматически, что позволяет нам делать так:
...
Еще несколько примеров - говорящих что в JS почти все - объекты:
...
Если вы желаете получить сам приметив из обертки, то можете использовать метод `valueOf()`
...
Получение примитива так же может происходить неявно, в случаи если это необходимо, Например
...
Как говорили выше - объекты являются динамическими, т.е. мы можем добавлять/удалять свойста после создания объекта.
...
В любой момент вы можете установить свойство `length` для обрезки массива. Когда вы расширяете массив, изменяя его свойство length, реальное количество элементов в массиве не увеличивается;

например, если вы установите свойство `length` в 3, когда оно равно 2, массив по-прежнему будет содержить только два элемента. Таким образом, свойство length ничего не говорит о количестве определённых значений в массиве. Также смотрите раздел Взаимосвязь свойства length с числовыми свойствами.
